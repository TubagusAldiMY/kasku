...
// migrateWrapper adalah thin interface untuk testability (idempotency test
// menjalankan Up() dua kali, butuh akses ke ErrNoChange).
...
// MigrateNoChange adalah sentinel yang ekspos `migrate.ErrNoChange` untuk
// pengecekan idempotency di test (Up() kedua kali return ErrNoChange).
