-- Adds runtime tenant objects that were planned in databaseScheme.md but were
-- missing from provisioned tenant schemas: sync_log plus an idempotency guard
-- for default category seeds.
...
BEGIN
    ...
    -- Remove duplicate default category rows before adding the partial unique
    -- index. This keeps repeated provision_tenant() calls idempotent.
    EXECUTE format('
...
BEGIN
    ...
    LOOP
        PERFORM public.ensure_tenant_runtime_objects(r.schema_name);
    END LOOP;
END $$;
