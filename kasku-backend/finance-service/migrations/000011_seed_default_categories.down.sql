-- 000011 rollback: soft-delete semua default category seeds yang ditambahkan,
-- drop helper function, dan restore provision_tenant() ke versi 000003 (tanpa seeding).

-- Soft-delete seeds di semua tenant yang ada (menggunakan fungsi yang dibuat di 000005)
DO $$
DECLARE
    r RECORD;
BEGIN
...
DROP FUNCTION IF EXISTS public.seed_default_categories(TEXT);

-- Restore provision_tenant() ke versi 000003 (tanpa seed call)
