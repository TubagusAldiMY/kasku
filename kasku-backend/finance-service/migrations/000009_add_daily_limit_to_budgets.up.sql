-- 000009: Add daily_limit_enabled column to budgets in all existing tenant schemas
-- and update provision_tenant() to include the column for new tenants.

-- 1. Backfill all existing tenant schemas with the new column.
DO $$
DECLARE
    r RECORD;
BEGIN
...
-- 2. Update provision_tenant() so new tenants get the column in their budgets table.
...
BEGIN
    ...
    -- budgets: per-category or global spending limits (includes daily_limit_enabled)
    EXECUTE format('
