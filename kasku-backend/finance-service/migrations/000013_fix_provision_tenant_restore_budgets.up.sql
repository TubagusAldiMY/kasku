-- 000013: Perbaiki regresi 000012 — provision_tenant() kehilangan tabel budgets.
--
-- Migrasi 000012 (extend dengan debts) me-redefine provision_tenant() dengan
-- menyalin body versi pra-budgets (000007) lalu menambah debts/debt_payments,
-- sehingga blok pembuatan tabel `budgets` (000008 + kolom daily_limit_enabled
-- dari 000009) ikut hilang. Akibatnya tenant yang di-provision SETELAH 000012
-- tidak punya tabel budgets → transaction-service gagal:
--   ERROR: relation "tenant_....budgets" does not exist (SQLSTATE 42P01)
--
-- Migrasi ini: (1) definisikan ulang provision_tenant() LENGKAP (semua tabel,
-- termasuk budgets), dan (2) backfill tabel budgets ke tenant lama yang belum punya.
...
BEGIN
    ...
    -- budgets: DIKEMBALIKAN (hilang sejak 000012). Termasuk daily_limit_enabled (000009).
    EXECUTE format('
    ...
    EXECUTE format('
        ...
        ON %I.budgets (user_id)
        WHERE is_deleted = false', v_schema);

    -- Tabel debts: catatan hutang & piutang per tenant (000012)
    EXECUTE format('
    ...
    EXECUTE format('
        CREATE INDEX IF NOT EXISTS idx_debts_user_direction
        ON %I.debts (user_id, direction, status)', v_schema);

    -- Tabel debt_payments: riwayat cicilan per debt (append-only) (000012)
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.debt_payments (
            id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    EXECUTE format('
        ...
        ON %I.debt_payments (debt_id, created_at DESC)', v_schema);
...
-- Backfill: buat tabel budgets untuk tenant lama yang belum punya (mis. tenant
-- yang di-provision setelah regresi 000012).
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        ...
        EXECUTE format(
            'CREATE INDEX IF NOT EXISTS budgets_user_active ON %I.budgets (user_id) WHERE is_deleted = false',
            r.schema_name
        );

        EXECUTE format(
            'GRANT SELECT, INSERT, UPDATE, DELETE ON %I.budgets TO kasku_transaction_svc, kasku_sync_svc',
            r.schema_name
        );
    END LOOP;
END $$;
