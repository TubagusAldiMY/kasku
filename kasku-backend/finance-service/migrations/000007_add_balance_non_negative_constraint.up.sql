-- 000007: Add CHECK (balance >= 0) to financial_accounts in all tenant schemas.
-- Defense-in-depth: prevents negative balance at the database level even if
-- application-layer validation is bypassed.

-- 1. Backfill all existing tenant schemas with the constraint.
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        BEGIN
            EXECUTE format(
                'ALTER TABLE %I.financial_accounts ADD CONSTRAINT financial_accounts_balance_non_negative CHECK (balance >= 0)',
                r.schema_name
            );
        EXCEPTION
...
-- 2. Update provision_tenant so new tenants also get the constraint.
