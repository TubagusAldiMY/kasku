-- 000008: Add budgets table to each tenant schema.
-- Budgets are owned by transaction-service domain but stored in the shared
-- tenant schema provisioned here (same pattern as investment_assets in 000003).

-- 1. Backfill all existing tenant schemas with the budgets table.
DO $$
DECLARE
    r RECORD;
BEGIN
...
-- 2. Update provision_tenant so new tenants also get the budgets table.
...
BEGIN
    ...
    -- budgets: per-category or global spending limits
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.budgets (
            id              UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
            user_id         UUID        NOT NULL,
