-- Rollback 000009: Drop daily_limit_enabled from all tenant schemas
-- and restore provision_tenant() to the 000008 version (without daily_limit_enabled).

-- 1. Drop column from all existing tenant schemas.
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER TABLE %I.budgets DROP COLUMN IF EXISTS daily_limit_enabled',
            r.schema_name
        );
    END LOOP;
END $$;

-- 2. Restore provision_tenant() to 000008 version (budgets without daily_limit_enabled).
