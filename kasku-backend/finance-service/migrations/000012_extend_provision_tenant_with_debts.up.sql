-- 000012: Extend provision_tenant() agar membuat tabel debts & debt_payments
-- per tenant schema untuk fitur pencatatan hutang/piutang.
...
BEGIN
    ...
    -- Tabel debts: catatan hutang & piutang per tenant
    EXECUTE format('
    ...
    -- Tabel debt_payments: riwayat cicilan per debt (append-only)
    EXECUTE format('
...
-- Backfill: tambah tabel debts & debt_payments ke tenant schemas yang sudah ada
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        ...
        EXECUTE format('
            CREATE INDEX IF NOT EXISTS idx_debts_user_direction
            ON %I.debts (user_id, direction, status)', r.schema_name);
        ...
        EXECUTE format('
            CREATE INDEX IF NOT EXISTS idx_debt_payments_debt_id
            ON %I.debt_payments (debt_id, created_at DESC)', r.schema_name);

        EXECUTE format('GRANT SELECT, INSERT, UPDATE, DELETE ON %I.debts, %I.debt_payments TO kasku_finance_svc, kasku_transaction_svc, kasku_investment_svc, kasku_sync_svc', r.schema_name, r.schema_name);
    END LOOP;
END $$;
