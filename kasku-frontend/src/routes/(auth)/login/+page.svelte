<script lang="ts">
	...
	async function handleGoogleLogin() {
		...
		try {
					...
					callback: async (response: { credential: string }) => {
						try {
							...
							if (result.success) {
								...
								error = result.error?.message || 'Login Google gagal.';
								fail(new Error(error ?? undefined));
							}
						} catch (err) {
			...
			if (!error) error = 'Login Google gagal. Coba lagi.';
			googleLoading = false;
		}
	}
	...
	<div class="space-y-4">
		...
		{#if googleClientId}
			...
			>
				...
				{/if}
				<span class="text-[14px] font-bold text-[#0a2e31]">
