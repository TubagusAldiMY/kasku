<script lang="ts">
			...
			const txForAcc = allTx
				...
				.map((t) => {
					...
					return {
						...
						category: catMap.get(t.category_id ?? '')?.name ?? (t.transaction_type === 'TRANSFER' ? 'Transfer' : 'Umum'),
						amount: signed,
	...
	async function handleDeleteAccount(id: string) {
		if (!confirm('Apakah Anda yakin ingin menghapus rekening ini? Seluruh riwayat transaksi terkait mungkin terpengaruh.')) return;
		try {
...
<div class="animate-in fade-in space-y-6 duration-500">
	...
	<div class="grid grid-cols-3 gap-3 sm:gap-6">
		<div class="space-y-1 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-6">
	...
	{:else if accounts.length === 0}
		<div class="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-gray-200 bg-gray-50/50 py-16">
			...
			<button onclick={() => (showAddModal = true)} class="font-bold text-[#217b84] hover:underline">Tambah Sekarang</button>
		</div>
	{:else}
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each accounts as acc (acc.id)}
				<div class="group relative overflow-hidden rounded-[2rem] border border-gray-100 bg-white p-6 shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-teal-900/5">
					<div class="absolute top-0 right-0 p-5">
						<div class="flex h-10 w-10 items-center justify-center rounded-2xl bg-gray-50 text-gray-400 transition-colors group-hover:bg-[#217b84] group-hover:text-white">
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					...
					<div class="space-y-3">
						<div>
							...
							<span class="text-[10px] font-bold tracking-widest text-gray-400 uppercase">{acc.account_type}</span>
						</div>
						<div class="pt-2">
...
{#if selectedAccount !== null}
	...
	<div
		class="fixed inset-x-0 bottom-0 z-50 flex max-h-[88dvh] flex-col rounded-t-[2rem] bg-white shadow-2xl lg:inset-y-0 lg:right-0 lg:left-auto lg:w-[420px] lg:max-h-none lg:rounded-none lg:rounded-l-[2rem]"
	...
	>
		...
		<div class="shrink-0 border-b border-gray-100 px-6 py-5">
			<div class="flex items-start justify-between gap-3">
				<div class="min-w-0">
					<p class="text-[10px] font-bold tracking-widest text-gray-400 uppercase">{selectedAccount.account_type}</p>
			...
			<div class="mt-4 flex items-end justify-between">
				<div>
					<p class="text-[10px] font-bold tracking-widest text-gray-400 uppercase">Saldo Saat Ini</p>
					<p class="text-2xl font-black text-[#0a2e31]">{formatCurrency(selectedAccount.balance)}</p>
				</div>
				<button
		...
		<div class="min-h-0 flex-1 overflow-y-auto">
			<div class="px-6 pt-5 pb-3">
				<p class="text-[10px] font-bold tracking-widest text-gray-400 uppercase">Riwayat Transaksi</p>
			</div>
				...
				<div class="space-y-2 px-4 pb-8">
					{#each selectedAccount.transactions as tx (tx.id)}
						<div class="flex items-center gap-3 rounded-2xl p-3 hover:bg-gray-50">
							<!-- Ikon tipe -->
							<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full {tx.type === 'INCOME' ? 'bg-green-50' : tx.type === 'TRANSFER' ? 'bg-blue-50' : 'bg-red-50'}">
								{#if tx.type === 'INCOME'}
									<svg class="h-4 w-4 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
										<path stroke-linecap="round" stroke-linejoin="round" d="M5 10l7-7m0 0l7 7m-7-7v18" />
									</svg>
								{:else if tx.type === 'TRANSFER'}
									<svg class="h-4 w-4 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
							...
							<!-- Nominal -->
							<span class="shrink-0 text-sm font-black {tx.amount > 0 ? 'text-green-600' : tx.type === 'TRANSFER' ? 'text-blue-500' : 'text-red-500'}">
...
{#if showAddModal}
	...
	>
		<div
			class="w-full max-h-[92dvh] overflow-y-auto rounded-t-[2rem] bg-white shadow-2xl sm:max-w-md sm:rounded-[2.5rem]"
		...
		>
			...
			<div class="space-y-5 p-5 sm:p-8">
				...
				<form onsubmit={handleAddAccount} class="space-y-4">
					<div>
						<label for="name" class="mb-1.5 block px-1 text-xs font-bold tracking-wider text-[#0a2e31] uppercase">Nama Rekening</label>
						<input
							id="name"
							type="text"
							required
					...
					<div>
						<span class="mb-1.5 block px-1 text-xs font-bold tracking-wider text-[#0a2e31] uppercase">Tipe Akun</span>
						<div class="grid grid-cols-3 gap-2">
							{#each accountTypes as type (type.id)}
								<button
									type="button"
									onclick={() => (newAccount.account_type = type.id)}
									class="flex flex-col items-center gap-2 rounded-2xl border-2 p-3 transition-all {newAccount.account_type === type.id ? 'border-[#217b84] bg-teal-50 text-[#217b84]' : 'border-gray-50 text-gray-400'}"
								>
									<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					...
					<div>
						<label for="balance" class="mb-1.5 block px-1 text-xs font-bold tracking-wider text-[#0a2e31] uppercase">Saldo Awal</label>
						<div class="relative">
